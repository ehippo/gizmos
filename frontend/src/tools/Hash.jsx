...
async function hashFile(file) {
  ...
  for (const algo of ['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512']) {
    ...
    results[algo] = Array.from(new Uint8Array(digest)).map(b => b.toString(16).padStart(2, '0')).join('')
  }
  return results
}
...
export default function HashTool() {
  ...
  const onDrop = e => {
    ...
    if (file) { setMode('File'); processFile(file) }
  }
  ...
  return (
    <ToolShell title="Hash Generator">
      <Toggle options={['Text', 'File']} value={mode} onChange={m => { setMode(m); setHashes({}) }} />
      ...
      {mode === 'Text' ? (
        ...
        >
          ...
          {fileName
            ? <><span className="hash-filename">{fileName}</span><span className="hash-filesize">{fileSize}</span></>
            : <span className="hash-drop-hint">Drop a file here or click to browse</span>
          }
          <input ref={fileRef} type="file" style={{ display: 'none' }} onChange={e => { if (e.target.files[0]) processFile(e.target.files[0]) }} />
        </div>
      )}
