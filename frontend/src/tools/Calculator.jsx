...
import { Toggle, Field, KVGrid, StatusBadge, CopyButton, IconButton, ToolShell } from '../components/ui'
...
function evaluate(expr) {
  ...
  const eat    = (ch) => { if (peek() === ch) { pos++; return true } return false }
  ...
  const parseMulDiv = () => {
    ...
    while (peek() === '*' || peek() === '/') {
      ...
      if (op === '*' && peek() === '*') { pos++; left = Math.pow(left, parsePow()); continue }
  ...
  const parsePow = () => {
    let base = parseUnary()
    if (peek() === '*' && src[pos+1] === '*') { pos += 2; return Math.pow(base, parseUnary()) }
    return base
  }
  const parseUnary = () => {
  ...
  const FUNCS = {
    ...
    round: Math.round, log: Math.log10, ln: Math.log, log2: Math.log2,
  ...
  const parsePrimary = () => {
    ...
    if (/[0-9.]/.test(peek() ?? '')) {
      let s = ''
      while (/[0-9.eE+\-]/.test(peek() ?? '') && !(s.length > 0 && /[eE]/.test(src[pos-1]) === false && (peek() === '+' || peek() === '-'))) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, minHeight: 0 }}>
      ...
      <div className="calc-history">
        {history.length === 0 && (
          <div style={{ color: 'var(--text3)', fontSize: 11, padding: '4px 0', textAlign: 'center' }}>
      ...
      <Field label="Expression — press Enter to evaluate, ↑ to recall">
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          />
          <IconButton icon={RotateCcw} label="Clear" onClick={() => setHistory([])} title="Clear history" />
        </div>
      </Field>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className="row">
        ...
        <Toggle
          ...
          value={fromBase}
          onChange={b => { setFromBase(b); convert(input, b) }}
        />
      </div>
