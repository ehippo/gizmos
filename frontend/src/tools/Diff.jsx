...
export default function DiffTool() {
  ...
  const numberedHunks = useMemo(() => {
    ...
    return hunks.map(h => {
      const ln = { ...h }
      if (h.type === 'del')  { ln.leftN = leftN++;  ln.rightN = null }
      if (h.type === 'add')  { ln.leftN = null;      ln.rightN = rightN++ }
      if (h.type === 'same') { ln.leftN = leftN++;   ln.rightN = rightN++ }
      return ln
    })
  }, [hunks, mode])
        ...
        <Field label="Diff" grow>
          {mode === 'Line' ? (
            ...
            <div className="diff-word-view">
              {hunks.map((h, i) =>
                h.type === 'add'  ? <ins key={i} className="diff-word-add">{h.text}</ins>  :
                h.type === 'del'  ? <del key={i} className="diff-word-del">{h.text}</del>  :
