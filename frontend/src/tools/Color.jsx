...
export default function ColorTool() {
  ...
  return (
    <ToolShell title="Color Converter">
      <Field label="Color input — #hex, rgb(), rgba(), hsl()">
        <div className="row gap-sm" style={{ alignItems: 'stretch' }}>
          ...
          <input
            ...
            onChange={e => convert(e.target.value)}
            style={{ width: 36, height: 28, padding: 2, cursor: 'pointer', border: '1px solid var(--border)', borderRadius: 'var(--radius)', background: 'var(--bg2)' }}
          />
        </div>
      </Field>
      ...
      <KVGrid rows={rows.length > 0 ? rows : [
        { key: 'HEX',  value: '—' },
        { key: 'RGB',  value: '—' },
        { key: 'RGBA', value: '—' },
        { key: 'HSL',  value: '—' },
