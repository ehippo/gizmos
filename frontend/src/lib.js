...
export function timestampNow() { return formatTime(new Date()) }

export function timestampConvert(input) {
  input = input.trim()
  const n = Number(input)
  const d = (input !== '' && !isNaN(n))
...
function formatTime(d) {
  ...
  const abs = Math.abs(diffMs), future = diffMs < 0
  let relative
  ...
  return {
    unix: Math.floor(d / 1000), unixMs: d.getTime(),
    iso8601: d.toISOString(), utc: d.toUTCString(),
...
export function parseColor(input) {
  ...
  if (rgb) { r = +rgb[1]; g = +rgb[2]; b = +rgb[3]; a = rgb[4] !== undefined ? +rgb[4] : 1 }
